...
Client consumes `domain`, `resolver`, `authoritative`, `cert`, `cert-sha256`,
`congestion-control`, and `keep-alive-interval`. Server consumes `domain`, `cert`, `key`,
`reset-seed`, `fallback`, and `max-connections`.
...
`cert-sha256` expects a 64-hex-character SHA-256 of the leaf certificate DER. If provided,
the client ignores `cert`.
